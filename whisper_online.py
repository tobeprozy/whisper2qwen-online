...
class FasterWhisperASR(ASRBase):
    ...
    sep = ""
    def __init__(self, lan, modelsize=None, model_dir=None, dev_id=0, logprob_threshold=-0.1, no_speech_threshold=0.6, logfile=sys.stderr):
        self.logfile = logfile
        ...
        self.model = self.load_model(modelsize, model_dir, dev_id)
    ...
    def load_model(self, modelsize=None, model_dir=None, dev_id = 0):
        ...
        model = bmwhisper_load_model(model_name = modelsize,
    ...
    def transcribe(self, audio, init_prompt=""):
        ...
        segments = bmwhisper_transcribe(self.model,
                                    audio=audio,
...
class OnlineASRProcessor:
    ...
    def __init__(self, qwen,asr, tokenizer=None, buffer_trimming=("segment", 15), logfile=sys.stderr):
...
def add_shared_args(parser):
    ...
    parser.add_argument('--backend', type=str, default="bm-whisper", choices=["bm-whisper"],help='Load only this backend for Whisper processing.')
...
def asr_factory(args, logfile=sys.stderr):
    ...
    asr = FasterWhisperASR(modelsize=size, lan=args.lan, model_dir=args.model_dir, dev_id=args.dev_id, logprob_threshold=args.logprob_threshold, no_speech_threshold=args.no_speech_threshold)
    e = time.time()
